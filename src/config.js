...
export const config = {
  ...
  pdfMaxPendingJobs: Math.max(0, Number(process.env.PDF_MAX_PENDING_JOBS || 20)),
  pdfPrewarmedSessions: Math.max(0, Number(process.env.PDF_PREWARMED_SESSIONS || 1)),
  pdfReuseSessions: String(process.env.PDF_REUSE_SESSIONS || "1").trim() === "1",
  pdfReuseSessionMaxUses: Math.max(1, Number(process.env.PDF_REUSE_SESSION_MAX_USES || 25)),
  pdfQueueWaitTimeoutMs: Math.max(0, Number(process.env.PDF_QUEUE_WAIT_TIMEOUT_MS || 10000)),
  ...
  pdfNetworkidleBudgetMs: Math.max(300, Number(process.env.PDF_NETWORKIDLE_BUDGET_MS || 800)),
  pdfAssetWaitTimeoutMs: Math.max(0, Number(process.env.PDF_ASSET_WAIT_TIMEOUT_MS || 400)),
